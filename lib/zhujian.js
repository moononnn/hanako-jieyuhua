...
import { generateSuggestions, generateAnswer, parseSuggestions, redactSecrets } from "./llm.js";
...
// ─── 问问小花：用解语花自己配置的模型直接回答 Hana 使用问题 ───
// 不注入任何真实会话（不污染聊天历史、不依赖某个助手的会话模型）；
// 弹窗问、弹窗答，完全独立。模型档位复用解语花设置页的三档配置。
export async function sendAskFlower(dataDir, bus, { text } = {}, modelSample = null) {
  const question = String(text || "").trim();
  if (!question) {
    return { ok: false, status: 400, error: "问题不能为空" };
  }
  if (question.length > 500) {
    return { ok: false, status: 400, error: "问题太长了，精简到 500 字以内吧" };
  }
  ...
  try {
    const answer = await generateAnswer(dataDir, question, {
      sampleFn,
      bus,
      maxTokens: 900,
    });
    ...
    if (!clean) {
      return { ok: false, status: 500, error: "小花没答出来，换个问法试试" };
    }
    console.log(`[解语花][ask-flower] 已回答 ${question.slice(0, 30)}`);
    return { ok: true, answer: clean };
  } catch (err) {
    const message = redactSecrets(err?.message || String(err));
    console.log(`[解语花][ask-flower] 回答失败 err=${message}`);
...
export function zhujianConfigPayload(cfg) {
  return {
    presentation: cfg?.presentation || "card",
    action: cfg?.action || "copy",
    askFlower: { enabled: cfg?.askFlower?.enabled === true },
  };
}
  ...
  proxyServer = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === "GET" && url === "/config") {
        // 悬浮球要读自己的开关配置（如问问小花入口显隐）；只回必要的、无密钥字段
        const cfg = getConfig(dataDir);
        return sendJson(res, 200, {
          ok: true,
          config: zhujianConfigPayload(cfg),
        });
      }
      ...
      if (req.method === "POST" && url === "/ask-flower") {
        const body = await readBody(req);
        const result = await sendAskFlower(dataDir, bus, { text: body?.text }, modelSample);
        return sendJson(res, result.ok ? 200 : (result.status || 400), result);
      }
