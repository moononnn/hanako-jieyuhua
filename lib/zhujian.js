...
import { readRecentMessages, readRecentAssistantMessages, selectRecentAssistantMessage, readAllMessages, buildContextText, buildTitleContext, resolveTargetSession, agentDisplayName, agentIdFromSessionPath, listNamedSessions, lastTurnInputEntryId, readBranchHistory } from "./session.js";
...
export async function resolveBallTarget(dataDir) {
  ...
  try {
    if (fs.existsSync(pinned.sessionPath)) {
      return {
        agentId: agentIdFromSessionPath(pinned.sessionPath) || pinned.agentId,
  ...
  // 钉住的会话已失效 → 只在它仍是当前旧目标时清除，避免覆盖用户刚选的新窗口。
