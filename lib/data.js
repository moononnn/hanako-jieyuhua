// 文件预算豁免：配置、待用推荐、提问与断联状态共用同一写锁和迁移归一化，拆开容易产生跨桶竞态。
// 解语花 — 数据读写模块
...
import {
  RESUME_AUTO_COOLDOWN_MS,
  RESUME_AUTO_MAX_CONSECUTIVE,
} from "./resume.js";
...
export const DEFAULT_CONFIG = {
  ...
  askFlower: {            // 问问小花（悬浮球里查 Hana 用法的快捷入口）
    enabled: false,       // 默认关闭，想用的人在设置页打开
  },
  ...
  resume: {                 // 断联续接：悬浮球检测到对话框异常停止时弹窗，一键继续（2026-08-27 新增）
    enabled: true,          // 总开关：断联时弹窗提醒（默认开）
    autoContinue: false,    // 自动续接：检测到断联自动发「继续」，不弹窗（默认关，保守）
  },
...
export function normalizeData(raw) {
  ...
  // 断联续接待办（未消费、未过期的保留；同会话只保留最新一条）
  const resumePending = {};
  if (data.resumePending && typeof data.resumePending === "object") {
    for (const [resumeId, raw] of Object.entries(data.resumePending)) {
      const entry = normalizeResumeEntry(raw, resumeId);
      if (!entry || entry.consumed) continue;
      const existing = resumePending[entry.sessionId];
      if (!existing || entry.ts > existing.ts) resumePending[entry.sessionId] = entry;
    }
  }
  // 同会话只留最新一条后，仍以 resumeId 为 key（consumeResume 按 resumeId 查找）
  const resumePendingByResumeId = {};
  for (const entry of Object.values(resumePending)) resumePendingByResumeId[entry.resumeId] = entry;
  // 防风暴状态（自动续接冷却 / 连续断联计数）
  const resumeState = {};
  if (data.resumeState && typeof data.resumeState === "object" && !Array.isArray(data.resumeState)) {
    for (const [sessionId, raw] of Object.entries(data.resumeState).slice(0, 200)) {
      if (!raw || typeof raw !== "object") continue;
      const state = {};
      if (Number.isFinite(raw.lastAutoTs) && raw.lastAutoTs > 0) state.lastAutoTs = raw.lastAutoTs;
      if (Number.isInteger(raw.consecutive) && raw.consecutive > 0) state.consecutive = raw.consecutive;
      if (Object.keys(state).length) resumeState[String(sessionId).slice(0, 200)] = state;
    }
  }
  // 自动续接通知（悬浮球短暂提示用，60 秒内有效）
  const resumeNotices = Array.isArray(data.resumeNotices)
    ? data.resumeNotices
        .map(normalizeResumeNotice)
        .filter(Boolean)
        .filter((notice) => Date.now() - notice.ts < RESUME_NOTICE_TTL_MS)
        .slice(-5)
    : [];
  return { config: cfg, pending, askPending, ballCache, pinnedTarget, lastRename, askSkipQueue, branchRefs, resumePending: resumePendingByResumeId, resumeState, resumeNotices };
}
...
export function normalizeConfig(raw) {
  ...
  if (raw.askFlower && typeof raw.askFlower === "object") {
    cfg.askFlower = { enabled: raw.askFlower.enabled === true };
  }
  ...
  if (raw.resume && typeof raw.resume === "object") {
    if (typeof raw.resume.enabled === "boolean") cfg.resume.enabled = raw.resume.enabled;
    if (typeof raw.resume.autoContinue === "boolean") cfg.resume.autoContinue = raw.resume.autoContinue;
  }
...
// ─── 断联续接待办（resumePending · 2026-08-27） ───
export const RESUME_TTL_MS = 30 * 60 * 1000;     // 待办 30 分钟过期（断联太久再弹没意义）
export const RESUME_MAX_PENDING = 5;             // 同时最多挂 5 条
export const RESUME_NOTICE_TTL_MS = 60 * 1000;   // 自动续接通知 60 秒内有效

export function normalizeResumeEntry(raw, fallbackResumeId = "") {
  if (!raw || typeof raw !== "object") return null;
  const resumeId = typeof raw.resumeId === "string" && raw.resumeId ? raw.resumeId : fallbackResumeId;
  const sessionPath = typeof raw.sessionPath === "string" ? raw.sessionPath.trim() : "";
  const ts = Number.isFinite(raw.ts) ? Number(raw.ts) : 0;
  if (!resumeId || !sessionPath || ts <= 0) return null;
  return {
    resumeId,
    ...
    agentName: String(raw.agentName || "").slice(0, 40),
    reason: String(raw.reason || "窗口断联了").slice(0, 60),
    source: raw.source === "stuck_turn" ? "stuck_turn" : "",
    ts,
...
export function normalizeResumeNotice(raw) {
  if (!raw || typeof raw !== "object") return null;
  const ts = Number.isFinite(raw.ts) ? Number(raw.ts) : 0;
  if (!ts) return null;
  return {
...
function pruneResumePendingObject(data, now = Date.now()) {
  let removed = 0;
  for (const [resumeId, entry] of Object.entries(data.resumePending || {})) {
    if (!entry || !Number.isFinite(entry.ts) || now - entry.ts >= RESUME_TTL_MS) {
      delete data.resumePending[resumeId];
      removed++;
    }
  }
  const keys = Object.keys(data.resumePending)
    .sort((a, b) => (data.resumePending[a].ts || 0) - (data.resumePending[b].ts || 0));
  while (keys.length > RESUME_MAX_PENDING) {
    delete data.resumePending[keys.shift()];
    removed++;
  }
  return removed;
}

export function createResumePending(dataDir, { agentId, sessionId, sessionPath, title, agentName, reason, source }) {
  return withDataLock(() => {
    const data = loadData(dataDir);
    pruneResumePendingObject(data);
    // 同会话已有未消费待办 → 刷新原因与时间，不重复弹
    for (const [resumeId, entry] of Object.entries(data.resumePending)) {
      if (entry && !entry.consumed && entry.sessionId === sessionId) {
        entry.ts = Date.now();
        ...
        saveData(dataDir, data);
        return { resumeId, entry, refreshed: true };
      }
    }
    const resumeId = "resume_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const entry = {
      resumeId,
      ...
      reason: String(reason || "窗口断联了").slice(0, 60),
      source: source === "stuck_turn" ? "stuck_turn" : "",
      ts: Date.now(),
      consumed: false,
      auto: false,
    };
    data.resumePending[resumeId] = entry;
    pruneResumePendingObject(data);
    saveData(dataDir, data);
    return { resumeId, entry, refreshed: false };
  });
}

export function listResumePending(dataDir) {
  const now = Date.now();
  return Object.values(loadData(dataDir).resumePending)
    .filter((entry) => entry && entry.consumed !== true && Number.isFinite(entry.ts) && now - entry.ts < RESUME_TTL_MS)
    .sort((a, b) => (a.ts || 0) - (b.ts || 0))
    .map(({ resumeId, agentId, sessionId, sessionPath, title, agentName, reason, source, ts }) => ({
      resumeId, agentId, sessionId, sessionPath, title, agentName, reason, source, ts,
    }));
}

export function consumeResume(dataDir, resumeId, { auto = false, failed = false } = {}) {
  return withDataLock(() => {
    const data = loadData(dataDir);
    const entry = data.resumePending[resumeId];
    if (!entry) return false;
    entry.consumed = true;
...
/** 会话恢复健康（用户自己发消息 / 回合正常完成 / 自动重试成功）时清掉未消费待办。 */
export function dismissResumeBySession(dataDir, sessionId) {
  if (!sessionId) return 0;
  return withDataLock(() => {
    const data = loadData(dataDir);
    let removed = 0;
    for (const entry of Object.values(data.resumePending || {})) {
      if (entry && !entry.consumed && entry.sessionId === sessionId) {
        entry.consumed = true;
        removed++;
      }
...
export function pruneResumePending(dataDir) {
  return withDataLock(() => {
    const data = loadData(dataDir);
    const removed = pruneResumePendingObject(data);
    if (removed) saveData(dataDir, data);
    return removed;
  });
}

// ── 断联防风暴状态（自动续接冷却 / 连续断联计数） ──
export function bumpResumeConsecutive(dataDir, sessionId) {
  if (!sessionId) return 0;
  return withDataLock(() => {
    const data = loadData(dataDir);
    const state = (data.resumeState && data.resumeState[sessionId]) || {};
    const consecutive = (Number.isInteger(state.consecutive) ? state.consecutive : 0) + 1;
    data.resumeState[sessionId] = { ...state, consecutive };
    saveData(dataDir, data);
    return consecutive;
  });
}

export function resetResumeConsecutive(dataDir, sessionId) {
  if (!sessionId) return;
  return withDataLock(() => {
    const data = loadData(dataDir);
    if (!data.resumeState || !data.resumeState[sessionId]) return;
    data.resumeState[sessionId].consecutive = 0;
    saveData(dataDir, data);
  });
}

/** 返回 { canAuto, consecutive }：冷却中或连续断联过多时 canAuto=false（该会话降级为只弹窗） */
export function checkResumeAutoAllowed(dataDir, sessionId) {
  const data = loadData(dataDir);
  const state = data.resumeState && data.resumeState[sessionId] ? data.resumeState[sessionId] : {};
  const consecutive = Number.isInteger(state.consecutive) ? state.consecutive : 0;
  const lastAutoTs = Number.isFinite(state.lastAutoTs) ? state.lastAutoTs : 0;
  if (consecutive >= RESUME_AUTO_MAX_CONSECUTIVE) return { canAuto: false, consecutive };
  if (lastAutoTs > 0 && Date.now() - lastAutoTs < RESUME_AUTO_COOLDOWN_MS) return { canAuto: false, consecutive };
  return { canAuto: true, consecutive };
}

export function markResumeAutoFired(dataDir, sessionId) {
  if (!sessionId) return;
  return withDataLock(() => {
    const data = loadData(dataDir);
    const state = (data.resumeState && data.resumeState[sessionId]) || {};
    data.resumeState[sessionId] = { ...state, lastAutoTs: Date.now() };
    saveData(dataDir, data);
  });
}

// ── 自动续接通知（悬浮球短暂提示） ──
export function pushResumeNotice(dataDir, { agentName = "", title = "" } = {}) {
  return withDataLock(() => {
    const data = loadData(dataDir);
    const notices = Array.isArray(data.resumeNotices) ? data.resumeNotices : [];
    notices.push({ agentName, title, ts: Date.now() });
    data.resumeNotices = notices
      .filter((notice) => notice && Number.isFinite(notice.ts) && Date.now() - notice.ts < RESUME_NOTICE_TTL_MS)
      .slice(-5);
    saveData(dataDir, data);
  });
}

export function listResumeNotices(dataDir) {
  const now = Date.now();
  return (loadData(dataDir).resumeNotices || [])
    .filter((notice) => notice && Number.isFinite(notice.ts) && now - notice.ts < RESUME_NOTICE_TTL_MS)
    .map(({ agentName, title, ts }) => ({ agentName, title, ts }));
}
