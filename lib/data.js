...
export function createResumePending(dataDir, { agentId, sessionId, sessionPath, title, agentName, reason }) {
  return withDataLock(() => {
    const data = loadData(dataDir);
...
export function listResumePending(dataDir) {
  ...
  return Object.values(loadData(dataDir).resumePending)
    ...
    .sort((a, b) => (a.ts || 0) - (b.ts || 0))
    .map(({ resumeId, agentId, sessionId, sessionPath, title, agentName, reason, ts }) => ({
      resumeId, agentId, sessionId, sessionPath, title, agentName, reason, ts,
    }));
}
