...
export async function callLLM(prompt, options = {}) {
  ...
  let response;
  try {
    response = await fetcher(url, { method: "POST", headers, body: JSON.stringify(body), signal: ctrl.signal });
  } finally {
...
export async function generateSuggestions(dataDir, prompt, {
  ...
  // Hana 档只把 provider/model 交给宿主总线，不读取或转发 Hana 的供应商 Key。
  if (bus && typeof bus.request === "function") {
    const result = await bus.request("utility:call-text", {
      messages: [{ role: "user", content: prompt }],
      providerId: plan.providerId,
      modelId: plan.modelId,
      ...(agentId ? { agentId } : {}),
      ...(sessionPath ? { sessionPath } : {}),
      operation: "jiegehua-suggestions",
    }, { timeoutMs: 30000 });
    return typeof result === "string" ? result : (result?.text ?? result?.content ?? result?.output ?? "");
  }
  throw new Error("指定 Hana 模型需要宿主的消息通道");
}
