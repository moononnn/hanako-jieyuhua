...
// 宿主自动重试耗尽。只负责识别与去重，不发送消息；发送/弹窗由上层处理。
// 事件源与提个醒同款（turn_end / error / session_status / auto_retry_start / auto_retry_end）。
//
// 行为约定（2026-08-27）：
...
export function buildResumeReason(errorMessage, { aborted = false, reason = "" } = {}) {
...
/** 悬浮球弹窗文案。 */
export function buildResumeCard({ agentName = "", sessionTitle = "", reason = "" } = {}) {
  const where = [agentName, sessionTitle].filter(Boolean).join(" · ");
  return {
    title: "🌸 窗口断联了",
    body: `${where ? `${where} ` : ""}${reason || "窗口断联了"}，点「继续」接上话头`,
  };
}
