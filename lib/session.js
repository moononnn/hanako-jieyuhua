  ...
  try {
    ...
    forEachLineFromEnd(sessionPath, (line) => {
      try {
        const entry = JSON.parse(line);
        const msg = entry.message && typeof entry.message === "object" ? entry.message : entry;
        if (msg?.role !== "user" || !extractConversationMessage(entry)) return false;
        const ts = entryTimestamp(entry);
  ...
  try {
    ...
    forEachLineFromEnd(sessionPath, (line) => {
      try {
        ...
        if (rawMessage.role !== "user" || !message) return false;
...
export async function listNamedSessions(bus, limit = 8) {
  let fallback = null;
  let fallbackByPath = null;
  const getFallback = () => {
    if (!fallback) fallback = listRecentSessions(Math.max(limit * 3, 24));
    return fallback;
  };
  const getFallbackByPath = () => {
    if (!fallbackByPath) {
      fallbackByPath = new Map(getFallback().map((item) => [path.normalize(item.sessionPath), item]));
    }
  ...
  try {
    ...
    const normalized = sessions
      ...
      .map((item) => {
        const inferredAgentId = agentIdFromSessionPath(item.path);
        const needsFallback = (!item.title && !item.firstMessage) || !item.modified || (!item.agentId && !inferredAgentId);
        const matched = needsFallback ? getFallbackByPath().get(path.normalize(item.path)) : null;
    ...
    return normalized.length ? normalized.slice(0, limit) : getFallback().slice(0, limit);
  } catch {
